package com.codersworld.rklib.pager
 ...
 * For infinite Scroll, you may use infinitePagerAdapter.
 */
class InfiniteViewPager : ViewPager {
    ...
    constructor(
        ...
        val a =
            context.theme.obtainStyledAttributes(attrs, R.styleable.InfiniteViewPager, 0, 0)
        try {
            isInfinite = a.getBoolean(R.styleable.InfiniteViewPager_isInfinite, false)
            isAutoScroll = a.getBoolean(R.styleable.InfiniteViewPager_autoScroll, false)
            wrapContent = a.getBoolean(R.styleable.InfiniteViewPager_wrap_content, true)
            interval = a.getInt(R.styleable.InfiniteViewPager_scrollInterval, 5000)
    ...
    val indicatorCount: Int
        get() = if (adapter is InfinitePagerAdapter<*>) {
            (adapter as InfinitePagerAdapter<*>).listCount
        } else {
            adapter?.count ?: 0
        }
