package com.codersworld.rklib.shapes.views;
...
import com.codersworld.rklib.shapes.ShapeView;
import com.codersworld.rklib.shapes.manager.ClipPathManager;

public class DottedCutCornerView extends ShapeView {
    ...
    public DottedCutCornerView(@NonNull Context context) {
        super(context);
        init(context, null);
    }

    public DottedCutCornerView(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    public DottedCutCornerView(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(Context context, AttributeSet attrs) {
        if (attrs != null) {
            final TypedArray attributes = context.obtainStyledAttributes(attrs, R.styleable.DottedCutCornerView);
            topLeftCutSize = attributes.getDimensionPixelSize(R.styleable.DottedCutCornerView_shape_dottedCutCorner_topLeftSize, (int) topLeftCutSize);
            topRightCutSize = attributes.getDimensionPixelSize(R.styleable.DottedCutCornerView_shape_dottedCutCorner_topRightSize, (int) topRightCutSize);
            bottomLeftCutSize = attributes.getDimensionPixelSize(R.styleable.DottedCutCornerView_shape_dottedCutCorner_bottomLeftSize, (int) bottomLeftCutSize);
            bottomRightCutSize = attributes.getDimensionPixelSize(R.styleable.DottedCutCornerView_shape_dottedCutCorner_bottomRightSize, (int) bottomRightCutSize);
            dotEdgePosition = attributes.getInteger(R.styleable.DottedCutCornerView_shape_edge_position, POSITION_NONE);
            dotRadius = attributes.getDimensionPixelSize(R.styleable.DottedCutCornerView_shape_dot_radius, (int) dotRadius);
            dotSpacing = attributes.getDimensionPixelSize(R.styleable.DottedCutCornerView_shape_dot_spacing, (int) dotSpacing);
            attributes.recycle();
        }
        super.setClipPathCreator(new ClipPathManager.ClipPathCreator() {
            @Override
            public Path clipPathCreator(int width, int height) {
